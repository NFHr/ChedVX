...
namespace ChedVX.UI
{
    ...
    {
        private int laneNumbers = 4;
        private Matrix GetLaneDrawingMatrix(Matrix baseMatrix, int laneNumber, bool flipY)
        {
            ...
            matrix.Translate(LaneWidth * laneNumber, 0, MatrixOrder.Append);
            matrix.Translate((laneNumber-1) * LaneWidth * 2, 0, MatrixOrder.Append);
            return matrix;
        }
        ...
        {
            base.OnPaint(pe);
            // Drawing the positive direction of the Y axis as tick increase (y = 0 is the bottom of control)
            var prevMatrix = pe.Graphics.Transform;
            pe.Graphics.Transform = GetDrawingMatrix(prevMatrix, true);
            ...
            for (int k = 1; k < 4; k++)
            {
                pe.Graphics.Transform = GetLaneDrawingMatrix(prevMatrix, k, true);
                ...
                using (var darkPen = new Pen(LaneBorderDarkColor, BorderThickness))
                {
                    for (int i = 0; i <= Constants.LanesCount; i++)
                    {
                        ...
                        pe.Graphics.DrawLine(i % 2 == 0 ? lightPen : darkPen, x, GetYPositionFromTick(HeadTick), x,
                            GetYPositionFromTick(tailTick));
                    }
                }
                ...
                using (var barPen = new Pen(BarLineColor, BorderThickness))
                {
                    ...
                    for (int i = HeadTick / (barTick / sigs[0].Denominator);
                         sigs.Count < 2 || i * barTick / sigs[0].Denominator <
                         sigs[1].Tick / firstBarLength * firstBarLength;
                         i++)
                    {
                    ...
                    for (int j = 1; j < sigs.Count; j++)
                    {
                        ...
                        for (int i = HeadTick - pos < 0 ? 0 : (HeadTick - pos) / (barTick / sigs[j].Denominator);
                             pos + i * (barTick / sigs[j].Denominator) < tailTick;
                             i++)
                        {
                            if (j < sigs.Count - 1 && i * barTick / sigs[j].Denominator >=
                                (sigs[j + 1].Tick - pos) / currentBarLength * currentBarLength) break;
                ...
                // Draw Notes
                var BTLongs = Notes.BTs
                    .Where(p => p.StartTick <= tailTick && p.StartTick + p.Duration >= HeadTick && !p.IsChip).ToList();

                // Button Chip Note
                foreach (var note in Notes.BTs.Where(
                             p => p.StartTick >= HeadTick && p.StartTick <= tailTick && p.IsChip))
                {
                ...
                // FX Chip Note
                foreach (var note in Notes.FXs.Where(
                             p => p.StartTick >= HeadTick && p.StartTick <= tailTick && p.IsChip))
                {
                ...
                */
                ...
                pe.Graphics.Transform = GetLaneDrawingMatrix(prevMatrix, k, false);
                ...
                using (var font = new Font("MS Gothic", 8))
                {
                    ...
                    for (int j = 0; j < sigs.Count; j++)
                    {
                        ...
                        {
                            if (j < sigs.Count - 1 && i * currentBarLength >=
                                (sigs[j + 1].Tick - pos) / currentBarLength * currentBarLength) break;
                    ...
                    {
                        foreach (var item in ScoreEvents.BpmChangeEvents.Where(p =>
                                     p.Tick >= HeadTick && p.Tick < tailTick))
                        {
                            var point = new PointF(rightBase, -GetYPositionFromTick(item.Tick) - strSize.Height);
                            pe.Graphics.DrawString(Regex.Replace(item.Bpm.ToString(), @"\.0$", "").PadLeft(3), font,
                                Brushes.Lime, point);
                        }
                    }
                    ...
                    using (var sigBrush = new SolidBrush(Color.FromArgb(216, 116, 0)))
                    {
                        ...
                        {
                            var point = new PointF(rightBase + strSize.Width,
                                -GetYPositionFromTick(item.Tick) - strSize.Height);
                            pe.Graphics.DrawString(string.Format("{0}/{1}", item.Numerator, item.Denominator), font,
                                sigBrush, point);
                        }
                    }
                    ...
                    {
                        foreach (var item in ScoreEvents.HighSpeedChangeEvents.Where(p =>
                                     p.Tick >= HeadTick && p.Tick < tailTick))
                        {
                            var point = new PointF(rightBase + strSize.Width * 2,
                                -GetYPositionFromTick(item.Tick) - strSize.Height);
                            pe.Graphics.DrawString(string.Format("x{0: 0.00;-0.00}", item.SpeedRatio), font,
                                highSpeedBrush, point);
                        }
                    }
                }
            }
